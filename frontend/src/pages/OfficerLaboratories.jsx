...
import { laboratoryAPI, doctorAPI } from '../api/api';
import { FaSearch, FaChevronLeft, FaChevronRight, FaCheckCircle, FaHospital, FaPlus } from 'react-icons/fa';
...
const OfficerLaboratories = () => {
    ...
    const fetchAllLabs = async () => {
        setLoading(true);
        try {
            const res = await laboratoryAPI.getAll();
            setLaboratories(res.data || []);
        } catch (error) {
            console.error("Error fetching labs:", error);
            Swal.fire({ icon: 'error', title: 'Error', text: 'Could not load laboratories list.', customClass: { popup: 'swal-modern-popup' } });
        } finally {
            setLoading(false);
        }
    };
    ...
    const getVisibleLabs = () => {
        ...
        return laboratories.filter(l => 
            ...
            l.owner_name?.toLowerCase().includes(lowerQ)
        );
    };

    const filteredLabs = getVisibleLabs();
    ...
    const handleToggleActive = async (labId, currentStatus) => {
        try {
            setLaboratories(prev => prev.map(l => l.id === labId ? { ...l, active: !currentStatus } : l));
            await laboratoryAPI.updateLab(labId, { active: !currentStatus });
            Toast.fire({ icon: 'success', title: `Laboratory ${!currentStatus ? 'Activated' : 'Deactivated'} Successfully` });
        } catch (error) {
        ...
        <Container fluid className="p-4 fade-in">
            ...
            <div className="text-center mb-4">
                ...
                <p className="mb-4" style={{ fontFamily: "'Google Sans', sans-serif", fontSize: '0.85rem', color: 'var(--luna-mid)', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '1px' }}>
                    Manage and update medical laboratories
                </p>
            </div>
                    ...
                    <div className="d-flex gap-3">
                        <div className="voice-assist-container d-flex align-items-center flex-grow-1" style={{ padding: '6px 10px', height: '52px' }}>
                            <input 
                                ...
                                placeholder="Search by Lab Name, Email, Mobile or Owner..." 
                                value={searchQuery}
                                onChange={(e) => {
                        ...
                        <button 
                            ...
                            style={{ height: '52px', padding: '0 25px', borderRadius: '50px', fontSize: '0.95rem' }}
                        >
            ...
            {loading ? (
                <div className="loading-container mt-5">
                    ...
                    <div className="font-google-sans" style={{fontWeight: 600}}>Loading Laboratories...</div>
                </div>
            ) : (
                <>
                                ...
                                {currentLabs.length > 0 ? (
                                        ...
                                        <tr key={lab.id}>
                                            <td className="text-start ps-4">
                                                <div className="doc-info-cell align-items-center">
                                                    <div className="avatar-box bg-light text-luna-mid rounded-circle" style={{minWidth:'36px', height:'36px', color: '#5E35B1'}}>
                                                        <FaHospital size={14}/>
                                                    </div>
                                                    <span className="doc-name mb-0" style={{fontSize: '0.9rem'}}>{lab.name || 'N/A'}</span>
                                                </div>
                                            </td>
                                            <td><span className="text-unified" style={{fontWeight: 500}}>{lab.email || 'N/A'}</span></td>
                                            <td>
                                                <span 
                                                    className="text-unified" 
                                                    style={{color: 'var(--luna-light)', cursor: 'pointer', textDecoration: 'underline', textUnderlineOffset: '2px'}}
                                                    onClick={() => handleOwnerClick(lab.owner)}
                                                    title="Click to view Owner Profile"
                                                >
                                                    {lab.owner_name || 'N/A'}
                                                </span>
                                            </td>
                                            <td><span className="text-unified">{lab.mobile || 'N/A'}</span></td>
                                            
                                            <td>
                                                <div className="d-flex justify-content-center">
                                                    <button 
                                                        className={`status-check-btn ${lab.active ? 'active-status' : 'inactive-status'}`}
                                                        onClick={() => handleToggleActive(lab.id, lab.active)}
                                                    >
                                            ...
                                            <td>
                                                <button className="btn-view-profile-capsule" onClick={() => navigate('/officer/view-business', { state: { labId: lab.id } })}>
                                                    View Lab
                                                </button>
                                            </td>
                                        </tr>
                                    ...
                                    <tr className="row-transparent">
                                        <td colSpan="6" className="no-results-view no-results-content">
                                            ...
                                            <p className="nr-subtitle">No laboratory matches your search criteria.</p>
                                            <div className="nr-divider"></div>
                                        </td>
                                    </tr>
                                )}
...
export default OfficerLaboratories;